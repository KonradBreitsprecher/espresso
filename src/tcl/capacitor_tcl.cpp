...
int tclcommand_gen_pot_from_mesh(ClientData data, Tcl_Interp *interp, int argc, char **argv) 
{
  ...
  if(argc < 3) { 
	  ...
	  if (argc != 8) {
		  ...
		  if (!ARG_IS_D(3, surface_prec)) {
			  Tcl_AppendResult(interp, "Expecting surface precision for boundary detection\n" , (char *)NULL);
			  return TCL_ERROR;
		  }
		  
		  if (!ARG_IS_I(4, num_iter)) {
			  Tcl_AppendResult(interp, "Expecting integer number of iterations of jacobi relaxation\n" , (char *)NULL);
			  return TCL_ERROR;
		  }
