...
# in all positions that are in the within the box.
  ...
  for { set i -$n_charges_l } { $i <= $n_charges_l } { incr i } {
	...
	#puts "$posx $posy $posz"
        part $n_induced_charges pos $posx $posy $posz q [expr $sigma*$area +0.1*([ t_random ]-0.5)] fix 1 1 1 type $type
...
proc dielectric_hexagonal_wall { args } {
  global n_induced_charges icc_areas icc_normals icc_epsilons icc_sigmas
  if { ![ info exists n_induced_charges ] } {
    set n_induced_charges 0
    set icc_areas [ list ]
  ...
  for { set argno 0 } { $argno < [ llength $args ] } { } {
    ...
    if { [ lindex $args $argno ] == "shift" } {
      incr argno
      set s1 [ expr 1.0 * [ lindex $args $argno ] ]
      incr argno
      set s2 [ expr 1.0 * [ lindex $args $argno ] ]
      incr argno
      continue
    }
    if { [ lindex $args $argno ] == "lattice_param" } {
      incr argno
      set lattice_param [ expr 1.0* [ lindex $args $argno ] ]
      incr argno
      continue
    }
    if { [ lindex $args $argno ] == "sigma" } {
    ...
    if { [ lindex $args $argno ] == "icc" } {
      incr argno
      set is_icc [expr [ lindex $args $argno ] ]
      incr argno
      continue
    }
    error "did not understand arg [ lindex args $argno ]"
...
# in all positions that are in the within the box.
  ...
  set d1 [expr 3.0 * $lattice_param]
  set d2 [expr sqrt(3.0) * $lattice_param]
  ...
  for { set i -$n_l1 } { $i <= $n_l1 } { set i [expr $i + $d1] } {
    for { set j -$n_l2 } { $j <= $n_l2 } { set j [expr $j + $d2] } {
      for { set k 0 } { $k < 4 } { incr k } {
        set l1 [expr $i+ [lindex [lindex $hcell $k] 0] + $s2]
	set l2 [expr $j+ [lindex [lindex $hcell $k] 1] + $s1]
	set posx [ expr $nx * $dist + $l1*$e_1_x + $l2*$e_2_x ]
	set posy [ expr $ny * $dist + $l1*$e_1_y + $l2*$e_2_y ]
	set posz [ expr $nz * $dist + $l1*$e_1_z + $l2*$e_2_z ]
	if { $posx >= 0 && $posx < $box_l_x && $posy >= 0 && $posy < $box_l_y && $posz >= 0 && $posz < $box_l_z } {
	  ...
	  if {$is_icc == 1} {
	    ...
	    part [setmd n_part] pos $posx $posy $posz fix 1 1 1 type $type
          }
  ...
  puts "Created dielectric hexagonal plane with $counter particles:\nn = \[$nx $ny $nz\], e1 = \[$e_1_x $e_1_y $e_1_z\], e2 = \[$e_2_x $e_2_y $e_2_z\], areas = $area" 
